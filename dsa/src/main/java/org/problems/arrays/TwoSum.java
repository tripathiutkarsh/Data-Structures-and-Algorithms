...
public class TwoSum {
    ...
    // Work in Progress (Not all Tests are passing)
    public static int[] twoSumUsingArray(int target, int[] nums) {
        ...
        // target might still be bigger than max
        if (target > max)
            max = target;
        ...
        for (int i = 0; i < nums.length; i++) {
            if (vault[nums[i]] == Integer.MIN_VALUE)
                vault[Math.abs(target - nums[i])] = i; // Using absolute value in order to avoid -ve index and thus ArrayIndexOutOfBoundsException
            else {
